...
export default function PairPage() {
  ...
  async function claimCode(e) {
    ...
    try {
      const resp = await fetch(`${SERVER}/api/pair/resolve?code=${encodeURIComponent(code.trim())}`);
      const data = await resp.json();
      if (!resp.ok || !data.ok) throw new Error(data?.error || "invalid code");
      setMac(data.mac);
      setStatus("Code accepted. You can view/edit notes below.");
    } catch (e) {
  ...
  async function saveNotes(mode) {
    ...
    try {
      const resp = await fetch(`${SERVER}/api/notes/${mac}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Pair-Code": code.trim() },
  ...
  function unpair() {
    setMac("");
    setNotes("");
    setStatus("Cleared. Enter your code again to continue.");
  }
    ...
    <div className="max-w-2xl mx-auto p-6">
      ...
      {!mac ? (
        ...
        <div className="space-y-4">
          <div className="text-sm text-gray-600">Device: <code>{mac}</code> &nbsp;|&nbsp; Using code: <code>{code}</code></div>
          <div className="flex gap-2">
            ...
            <button onClick={unpair} className="px-3 py-2 border rounded ml-auto">Clear Form</button>
          </div>
          <textarea
            value={notes}
