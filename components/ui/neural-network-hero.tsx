...
const fragmentShader = `
  ...
  void main() {
    // ZOOMED OUT VISIBILITY FIX: 3.0 scale
    vec2 uv = vUv * 3.0 - 1.5; uv.y *= -1.0;
    
    // INCREASED TIME MULTIPLIER FOR MOVEMENT VISIBILITY
    float speed = iTime * 2.0;
    gl_FragColor = cppn_fn(uv, 0.1 * sin(0.3 * speed), 0.1 * sin(0.69 * speed), 0.1 * sin(0.44 * speed));
  }
`;
...
function ShaderPlane() {
    ...
    return (
        // Centered and sufficiently large
...
function ShaderBackground() {
    ...
    return (
        <div className="absolute inset-0 -z-10 w-full h-full bg-[#020617]">
            {/* Fallback Gradient: Darker Blue/Slate for Contrast with Screen Blend */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-blue-900/40 via-slate-950 to-black z-0 opacity-100" />
            ...
            <Canvas
                ...
                style={{
                    ...
                    left: 0,
                    zIndex: 1,
                    mixBlendMode: 'screen', // SCREEN Mode ensures Glow on Dark BG
                    opacity: 1.0
                }}
            >
                <ShaderPlane />
...
export default function NeuralNetworkHero({
    ...
    useGSAP(
        () => {
            ...
            if (typeof SplitText !== 'undefined') {
                ...
                gsap.to(headerRef.current, { autoAlpha: 1, y: 0, duration: 1 });

                if (badgeRef.current) { gsap.set(badgeRef.current, { autoAlpha: 0, y: -8 }); gsap.to(badgeRef.current, { autoAlpha: 1, y: 0, duration: 0.5 }); }
                if (paraRef.current) { gsap.set(paraRef.current, { autoAlpha: 0, y: 8 }); gsap.to(paraRef.current, { autoAlpha: 1, y: 0, duration: 0.5, delay: 0.2 }); }
                if (ctaRef.current) { gsap.set(ctaRef.current, { autoAlpha: 0, y: 8 }); gsap.to(ctaRef.current, { autoAlpha: 1, y: 0, duration: 0.5, delay: 0.4 }); }
            }
        },
        { scope: sectionRef },
    );
    ...
    return (
        <section ref={sectionRef} className="relative h-screen w-full overflow-hidden bg-black">
            ...
            <div className="relative mx-auto flex max-w-7xl flex-col lg:flex-row items-center gap-12 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16 h-full justify-center">

                {/* Left Column: Text */}
                <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left gap-6 z-10 w-full">
                    ...
                    <h1 ref={headerRef} className="text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl">
                        {title}
                    </h1>
                    ...
                    <div ref={ctaRef} className="flex flex-wrap items-center justify-center lg:justify-start gap-3 pt-2">
                    ...
                    <ul ref={microRef} className="mt-8 flex flex-wrap justify-center lg:justify-start gap-6 text-xs font-extralight tracking-tight text-white/60">
                ...
                {/* Right Column: Calculator Image - LARGE SIZE */}
                <div className="flex-1 w-full flex justify-center lg:justify-end relative z-10">
                    <div className="relative w-[350px] sm:w-[500px] lg:w-[700px] h-[400px] sm:h-[600px] lg:h-[750px] -mr-10">
                        <Image
                            src="/calcai-new.png"
                            alt="CalcAI Calculator"
                            fill
                            className="object-contain drop-shadow-2xl scale-125 lg:scale-150"
                            priority
                        />
                    </div>
                </div>
